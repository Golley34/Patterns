"use strict";
let animals = [];
const userCode = (creator) => {
    while (true) {
        const userNameAnimal = prompt("Введите имя животного");
        if (userNameAnimal === null || userNameAnimal.trim() === "") {
            break;
        }
        const userAgeAnimal = prompt("Введите возраст животного");
        ...
        const animal = creator.returnAnimal(userNameAnimal, userAgeAnimalNumber);
        animals.push(animal);
        break;
    }
};
const choiseStrategy = (select) => {
    ...
    while (true) {
        ...
        if (action === null) {
            ...
            if (animals.length === 0) {
                ...
                let select = prompt(selectAnimal);
                if (select === null) {
                    break;
                }
